...
// LoadDocument loads an existing DAO object by its scope and ID
func LoadDocument(ctx context.Context, api *eos.API, scope string, ID uint64) Document {
	var objects []Object
	...
	return ToDocument(objects[0])
}

// LoadDocuments from a provided eos.API and scope
func LoadDocuments(ctx context.Context, api *eos.API, scope string) []Document {
	var docs []Document
	objects := LoadObjects(ctx, api, scope)
	docs = make([]Document, len(objects))
	for index, object := range objects {
		docs[index] = ToDocument(object)
	}
	return docs
}

// Document is a generic object from the objects table
...
// ToDocument Converts a very generic Object to one of type DAO Object
func ToDocument(objs Object) Document {

	var daoObject Document
